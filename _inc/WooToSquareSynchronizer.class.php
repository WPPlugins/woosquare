        ...
        if ($product_variations) {   //Variable Product
			...
			foreach($setvariationformultupleattr as $mult_attr){
				...
				foreach($mult_attr as $attr){
					...
					$getingattrname .= $getingattrnamedata[0].'['.$getingattrval[0].'],';
				}
			...
			$datavariationssecon = $datavariations;
			foreach($datavariations as $key => $vall){
				if($key != 0 and $key != 1  ){
					unset($datavariationssecon[$key]);
				}
			}
			$data['variations'] = array();
			$data['variations'] = $datavariationssecon;	
        } else {    //Simple Product
